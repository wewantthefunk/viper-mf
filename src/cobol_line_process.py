...
from util import *
from cobol_verb_process import process_verb, check_valid_verb
from cobol_util import prep_copybook
from codegen import CodeWriter
from translation_context import TranslationContext

# Data division and var state now live on TranslationContext

def process_identification_division_line(line: str, name: str, context: TranslationContext):
    context.var_init_list = []
...
def process_environment_division_line(line: str, current_section: str, name: str, current_line, next_few_lines, context: TranslationContext, writer: CodeWriter):
    ...
    if line == FILE_CONTROL_SECTION:
        ...
        writer.write("# " + current_section + NEWLINE)
    elif tokens[0] == SELECT_KEYWORD:
        create_file_variable(tokens, name, next_few_lines, current_section, writer)
        ...
        create_class_variable(tokens, name, next_few_lines, current_section, writer, context)
    elif tokens[0] == SYMBOLIC_KEYWORD:
        create_symbolic_variable(tokens, name, next_few_lines, current_section, current_line, context.args, writer, context)

    return [line, current_line, name, current_section, next_few_lines, context.args]

def create_symbolic_variable(tokens, name: str, next_few_lines, current_section: str, current_line: LexicalInfo, args, writer: CodeWriter, context: TranslationContext = None):
    count = 0
    skip = False
    skip2 = False
    ...
    for token in tokens:
        ...
        create_variable("01 " + symbol_name + SPACE + "PIC X(1) VALUE X'" + hex(value).upper()[2:] + "'.", current_line, name, current_section, [], args, False, writer, context)

        count = count + 1

def create_class_variable(tokens, name: str, next_few_lines, current_section: str, writer: CodeWriter, context: TranslationContext):
    done_class_line = False
    ...
    writer.write(writer.indent(2) + "result = Add_Variable(" + SELF_REFERENCE + name + MEMORY + COMMA + SELF_REFERENCE + "_DataDivisionVars,'" + tokens[1] + "', " \
         + str(len(val)) + ", '" + "X" + "','" + EMPTY_STRING + "','" + EMPTY_STRING + "')" + NEWLINE)
    writer.write(writer.indent(2) + SELF_REFERENCE + "_DataDivisionVars = result[0]" + NEWLINE)
    writer.write(writer.indent(2) + SELF_REFERENCE + name + MEMORY + " = result[1]" + NEWLINE)
    ...
    context.var_init_list.append([COBOL_VERB_MOVE, SINGLE_QUOTE + value + SINGLE_QUOTE, EMPTY_STRING, tokens[1]])

def create_file_variable(tokens, name: str, next_few_lines, current_section: str, writer: CodeWriter):
    ...
    writer.write(writer.indent(2) + "result = Add_File_Variable(" + SELF_REFERENCE + "_FILE_CONTROLVars, '" + tokens[1] + "','" + assign \
        + "','" + organization + "','" + access + "','" + record_key + "','" + file_status + "')" + NEWLINE)
    writer.write(writer.indent(2) + SELF_REFERENCE + "_FILE_CONTROLVars = result" + NEWLINE)

def process_data_division_line(line: str, current_section: str, name: str, current_line: LexicalInfo, next_few_lines, context: TranslationContext, writer: CodeWriter):
    if line in DATA_DIVISION_SECTIONS:
        current_section = line
        current_line.first_line_section = True
        current_line.highest_ws_level = 99
        context.data_division_var_stack = []
        context.data_division_level_stack = []
        context.data_division_cascade_stack = []
        current_line.skip_the_next_lines = 0
        writer.write("# " + current_section + NEWLINE)
    else:
        tokens = parse_line_tokens(line, SPACE, EMPTY_STRING, True)
        if line.startswith(FD_KEYWORD) or line.startswith(SD_KEYWORD):
            context.data_division_file_record = tokens[1]
            if line.startswith(SD_KEYWORD):
                context.data_division_file_record = context.data_division_file_record + SORT_IDENTIFIER
        elif line.startswith(COPYBOOK_KEYWORD):
            insert_copybook(writer, line.replace(COPYBOOK_KEYWORD, EMPTY_STRING).strip(), current_line, name, current_section, next_few_lines, context)
        else:
            ...
            create_variable(line, current_line, name, current_section, next_few_lines, context.args, False, writer, context)

            if context.data_division_file_record != EMPTY_STRING and tokens[0].isnumeric():
                fr = tokens[1]
                if len(context.data_division_var_stack) > 0:
                    fr = context.data_division_var_stack[0]
                writer.write("# this is where we will associate the record " + tokens[1] + " to the file " + context.data_division_file_record + NEWLINE)
                writer.write(writer.indent(2) + "self._FILE_CONTROLVars = Set_File_Record(self._FILE_CONTROLVars, '" + context.data_division_file_record + "','" + fr + "')" + NEWLINE)
                context.data_division_file_record = EMPTY_STRING

    return [line, current_section, name, current_line]

def process_procedure_division_line(line: str, name: str, current_line: LexicalInfo, next_few_lines, context: TranslationContext, writer: CodeWriter):
    ...
    if temp_tokens[len(temp_tokens) - 1] == PERIOD:
        writer.write(writer.indent(level) + SELF_REFERENCE + "debug_line = '" + current_line.current_line_number + "'" + NEWLINE)
        fix_parens(temp_tokens, temp_tokens[0], temp_tokens[len(temp_tokens) - 1])
        level = process_verb(temp_tokens, name, True, level, context.args, current_line, next_few_lines, writer, context)
    else:
...
def create_variable(line: str, current_line: LexicalInfo, name: str, current_section: str, next_few_lines, args, is_eib=False, writer: CodeWriter = None, context: TranslationContext = None):
    if context is None:
        raise ValueError("create_variable requires context")
    ...
    if REDEFINES_KEYWORD in tokens:
        ...
        if current_line.redefines not in context.data_division_redefines_stack:
            context.data_division_redefines_stack.append(current_line.redefines)
    elif int(tokens[0]) <= int(current_line.redefines_level):
        if len(context.data_division_redefines_stack) > 0:
            context.data_division_redefines_stack.pop()
        if len(context.data_division_redefines_stack) > 0:
            current_line.redefines = context.data_division_redefines_stack[len(context.data_division_redefines_stack) - 1]
        else:
        ...
        if not hard_cascade_type:
            ...
            cascade_data_type = EMPTY_STRING
            if len(context.data_division_cascade_stack) > 0:
                context.data_division_cascade_stack.pop()
    ...
    if len(tokens) == 2 or (PIC_CLAUSE not in tokens and POINTER_CLAUSE not in tokens):  
        new_level = tokens[0]
        while len(context.data_division_level_stack) > 0 and int(new_level) <= int(context.data_division_level_stack[len(context.data_division_level_stack) - 1]):
                context.data_division_level_stack.pop()
                context.data_division_var_stack.pop()
                if len(context.data_division_cascade_stack) > 0:
                    context.data_division_cascade_stack.pop()

        if len(context.data_division_cascade_stack) > 0:
            if context.data_division_cascade_stack[len(context.data_division_cascade_stack) - 1] != cascade_data_type and cascade_data_type not in tokens:
                cascade_data_type = context.data_division_cascade_stack[len(context.data_division_cascade_stack) - 1]

        if len(context.data_division_var_stack) == 0:
            context.data_division_level_stack.append(tokens[0])
            context.data_division_var_stack.append(tokens[1])
            context.data_division_cascade_stack.append(cascade_data_type)
            current_line.highest_var_name = context.data_division_var_stack[len(context.data_division_var_stack) - 1]
            current_line.highest_ws_level = int(context.data_division_level_stack[len(context.data_division_level_stack) - 1])
        ...
        if current_line.highest_var_name == EMPTY_STRING:
            ...
            if (tokens[1] not in context.data_division_var_stack):
                context.data_division_var_stack.append(tokens[1])
                context.data_division_level_stack.append(tokens[0])
                context.data_division_cascade_stack.append(cascade_data_type)
        elif int(tokens[0]) == int(context.data_division_level_stack[len(context.data_division_level_stack) - 1]):
            if len(context.data_division_var_stack) > 0:
                context.data_division_var_stack.pop()
            if len(context.data_division_level_stack) > 0:
                context.data_division_level_stack.pop()
            if len(context.data_division_cascade_stack) > 0:
                context.data_division_cascade_stack.pop()
            if len(context.data_division_var_stack) == 0:
            ...
            if tokens[1] not in context.data_division_var_stack:
                ...
                context.data_division_var_stack.append(tokens[1])
                context.data_division_cascade_stack.append(cascade_data_type)
        elif len(context.data_division_var_stack) > 0:            
            if len(context.data_division_var_stack) == 0:
                context.data_division_var_stack.append(tokens[1])
                context.data_division_level_stack.append(tokens[0])
                context.data_division_cascade_stack.append(cascade_data_type)
        ...
        while len(context.data_division_var_stack) > 0:
            if int(context.data_division_level_stack[len(context.data_division_level_stack) - 1]) >= int(tokens[0]):
                if len(context.data_division_var_stack) > 0:
                    context.data_division_var_stack.pop()
                if len(context.data_division_level_stack) > 0:
                    context.data_division_level_stack.pop()
                ...
                if len(context.data_division_var_stack) > 0:
                    current_line.highest_var_name = context.data_division_var_stack[len(context.data_division_var_stack) - 1]
                    if len(context.data_division_cascade_stack) > 0:
                        current_line.cascade_data_type = context.data_division_cascade_stack[len(context.data_division_cascade_stack) - 1]
    ...
    writer.write(writer.indent(2) + SELF_REFERENCE + variable_list + " = Add_Variable(" + memory_name + "," + SELF_REFERENCE + variable_list + ",'" + v_name + "', " \
    ...
    if pic_clause_override:
        ...
        context.data_division_var_stack[len(context.data_division_var_stack) - 1] = v_name
    ...
    if VALUE_CLAUSE in tokens or VALUES_CLAUSE in tokens or current_line.cascade_init_value != EMPTY_STRING:
        ...
        if value_index == len(tokens) - 1:
            ...
            context.var_init_list.append([COBOL_VERB_MOVE, init_val, EMPTY_STRING, v_name])
        else:
            for x in range(value_index, len(tokens) - 1):
                ...
                if init_val == THRU_KEYWORD:
                    ...
                    while val < end:
                        if is_hex:
                            context.var_init_list.append([COBOL_VERB_MOVE, SINGLE_QUOTE + HEX_PREFIX + hex(val)[2:].upper() + SINGLE_QUOTE, EMPTY_STRING, v_name])
                        elif is_char:
                            context.var_init_list.append([COBOL_VERB_MOVE, SINGLE_QUOTE + char_prefix + pad_char(char_pad_len - len(str(val)), ZERO) + str(val) + SINGLE_QUOTE, EMPTY_STRING, v_name])
                        else:
                            context.var_init_list.append([COBOL_VERB_MOVE, SINGLE_QUOTE + str(val) + SINGLE_QUOTE, EMPTY_STRING, v_name])
...
def create_index_variables(vars, name: str, writer: CodeWriter):
    for var in vars:
        writer.write(writer.indent(2) + SELF_REFERENCE + var[3] + " = Add_Variable(" + SELF_REFERENCE + name + MEMORY + "," + SELF_REFERENCE + var[3] + ",'" + var[0] + "', " \
            + "10, '9','" + var[1] + "','',0,0,'','" + var[2] + "')[0]" + NEWLINE)

def allocate_variables(current_line: LexicalInfo, name: str, writer: CodeWriter):
    writer.write(writer.indent(2) + "result = Allocate_Memory(" + SELF_REFERENCE + "_DataDivisionVars," + SELF_REFERENCE + name + MEMORY + ")" + NEWLINE)
    writer.write(writer.indent(2) + SELF_REFERENCE + "_DataDivisionVars" + SPACE + EQUALS + SPACE + "result[0]" + NEWLINE)
    writer.write(writer.indent(2) + SELF_REFERENCE + name + MEMORY + SPACE + EQUALS + SPACE + "result[1]" + NEWLINE)

def init_vars(name: str, context: TranslationContext, writer: CodeWriter):
    level = context.lexical_info.level
    context.lexical_info.level = 2
    for vil in context.var_init_list:
        process_verb(vil, name, True, 2, context.args, context.lexical_info, [], writer, context)
    context.lexical_info.level = level
...
def insert_copybook(writer: CodeWriter, copybook, current_line: LexicalInfo, name, current_section, next_few_lines, context: TranslationContext):
    ...
    if len(lines) > 0:
        writer.write("# Inserted Copybook: " + copybook_name + NEWLINE)
    ...
    for line in lines:
        ...
        if skip_the_next_lines_count == current_line.skip_the_next_lines:
            ...
            continue
        create_variable(line, current_line, name, current_section, next_few_lines, context.args, is_eib, writer, context)
    ...
    if is_eib:
        writer.write(writer.indent(2) + "result = Allocate_Memory(self.EIBList,self.EIBMemory)\n")
        writer.write(writer.indent(2) + "self.EIBList = result[0]\n")
        writer.write(writer.indent(2) + "self.EIBMemory = result[1]\n")
