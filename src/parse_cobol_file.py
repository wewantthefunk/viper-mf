...
def process_line(line: str, current_division: str, name: str, current_line: LexicalInfo, next_few_lines):
    ...
    if current_division == COBOL_DIVISIONS[IDENTIFICATION_DIVISION_POS] \
        ...
        append_file(name + PYTHON_EXT, pad(len(INDENT) * 2) + SELF_REFERENCE + UNDERSCORE + EIB_VARIABLE_LIST + "Vars = []" + NEWLINE)
        ...
        append_file(name + PYTHON_EXT, pad(len(INDENT) * 2) + "result = Add_Variable(self.SPECIALREGISTERSMemory, self._INTERNALVars, 'SORT-RETURN', 4, '9', 'SORT-RETURN', '', 0, 0, '', '01')" + NEWLINE)
